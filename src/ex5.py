...
# ダッシュボード表示用関数(総件数、未対応件数、未対応率、SLA違反件数、SLA違反率)
def create_dashboard(df, unresolved_df, sla_df):
    total =len(df)
    unresolved = len(unresolved_df)
    sla = len(sla_df)

    unresolved_rate = unresolved / total * 100 if total else 0
    sla_rate = sla / total * 100 if total else 0

    dashboard = pd.DataFrame({
        "指標": ["総件数", "未対応件数", "未対応率(%)", "SLA違反件数", "SLA違反率(%)"],
        "値": [total, unresolved, round(unresolved_rate, 1), sla, round(sla_rate, 1)]
    })
    return dashboard
...
def export_to_excel(output_path, df, status_summary, priority_summary, assignee_summary, unresolved_df, status_priority, daily_trend, unresolved_over_3days, dashboard_df):
    try:
        with pd.ExcelWriter(output_path, engine="xlsxwriter") as writer:
            ...
            dashboard_df.to_excel(writer, sheet_name="ダッシュボード", index=False)
    # 例外処理
...
def main():
    ...
    dashboard_df = create_dashboard(df, unresolved_df, unresolved_over_3days_df)

    export_to_excel(output_path, df, status_summary, priority_summary, assignee_summary, unresolved_df, status_priority_df, daily_trend_df, unresolved_over_3days_df, dashboard_df)
